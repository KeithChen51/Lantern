...
- 可以使用 Markdown 表格呈现“条件 / 建议动作 / 说明”这类对比，但只在表格比列表更清楚时使用。
- 表格必须使用合法 GFM 表格语法，分隔行只能使用英文半角连字符，例如：|---|---|。不要使用中文长横线、全角横线或装饰线。
- 表格控制在 2-4 列、3-6 行，单元格使用短句，不要把整段话塞进表格。
- 表格前后保留空行，表格之外继续使用短标题和分点说明。
`;
}
